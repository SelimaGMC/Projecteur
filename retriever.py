...
def format_structured_data_to_docs(sql_data: dict, url: str) -> list[Document]:
    ...
    if fiche:
        ...
        docs.append(Document(page_content=fiche_text, metadata={"source": url, "type": "fiche_technique"}))
    ...
    if distribution:
        ...
        docs.append(Document(page_content=distrib_text, metadata={"source": url, "type": "distribution"}))
        
    # 3. Distinctions
    distinctions = sql_data.get("distinctions", [])
    if distinctions:
        distinc_text = "Distinctions et récompenses :\n" + "\n".join([f"- {prix}" for prix in distinctions])
        docs.append(Document(page_content=distinc_text, metadata={"source": url, "type": "distinctions"}))
...
def build_knowledge_base(film_urls: list[str], retriever_dir=RETRIEVER_DIR, embedding=OLLAMA_EMBEDDING) -> VECT_STORE | None:
    ...
    for i, url in enumerate(film_urls):
        try:
            ...
            if text:
                chunks = splitter.split_documents([Document(page_content=text, metadata={"source": url, "type": "synopsis_histoire"})])
                all_chunks.extend(chunks)
                
            # --- TRAITEMENT DES DONNÉES STRUCTURÉES ---
            if sql_data:
                structured_docs = format_structured_data_to_docs(sql_data, url)
